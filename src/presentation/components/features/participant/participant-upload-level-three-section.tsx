...
type ParticipantUploadLevelThreeSectionProps = {
  eventId: string;
  level1Id: string;
  level2Id: string;
  level1Name: string;
...
export function ParticipantUploadLevelThreeSection({
  eventId,
  level1Id,
  level2Id,
  level1Name,
  level2Name,
}: ParticipantUploadLevelThreeSectionProps) {
  const { subcategoriesByCategory, loading, error, retryEventContext } =
    useParticipantUploadEventContext(eventId, undefined, "upload", [level2Id]);
  ...
  return (
    <section className="rounded-3xl border border-[#1E1E1E] bg-[#121212] p-5 sm:p-6 md:p-8 xl:p-10">
      ...
      {!loading && !error ? (
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {filteredItems.map((item) => (
            <Link
              key={item.id}
              href={`/participante/subir/${eventId}/formulario/${level1Id}/${item.id}?l1=${encodeURIComponent(level1Name)}&l2=${encodeURIComponent(level2Name)}&l2id=${encodeURIComponent(level2Id)}&final=${encodeURIComponent(item.name)}`}
            ...
            >
              ...
              <p className="mt-2 text-sm text-[#9E9E9E]">
                Especialidad final para subir la maqueta
              </p>
      ...
      {!loading && !error && level3Items.length === 0 ? (
        <p className="mt-6 rounded-xl border border-[#2D2D2D] bg-[#151515] px-4 py-3 text-sm text-[#9C9C9C]">
          Esta subcategoria no tiene especialidades finales disponibles.
        </p>
      ) : null}
      {!loading && !error && level3Items.length > 0 && filteredItems.length === 0 ? (
        <p className="mt-6 rounded-xl border border-[#2D2D2D] bg-[#151515] px-4 py-3 text-sm text-[#9C9C9C]">
      ...
      <div className="mt-6">
        <Link
          href={`/participante/subir/${eventId}/nivel-2/${level1Id}?l1=${encodeURIComponent(level1Name)}`}
