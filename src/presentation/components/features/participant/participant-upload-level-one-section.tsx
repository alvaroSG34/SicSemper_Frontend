...
type ParticipantUploadLevelOneSectionProps = {
  eventId: string;
};

export function ParticipantUploadLevelOneSection({
  eventId,
  ...
  return (
    <section className="rounded-3xl border border-[#1E1E1E] bg-[#121212] p-5 sm:p-6 md:p-8 xl:p-10">
      ...
      {!loading && !error ? (
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {filteredItems.map((item) => {
            ...
            const href = isLeaf
              ? `/participante/subir/${eventId}/formulario/${item.id}/${item.id}?l1=${encodeURIComponent(item.name)}&final=${encodeURIComponent(item.name)}`
              : `/participante/subir/${eventId}/nivel-2/${item.id}?l1=${encodeURIComponent(item.name)}`;
      ...
      {!loading && !error && eventCategories.length === 0 ? (
        <p className="mt-6 rounded-xl border border-[#2D2D2D] bg-[#151515] px-4 py-3 text-sm text-[#9C9C9C]">
          Este evento no tiene categorias disponibles para subir maquetas.
        </p>
      ) : null}
      {!loading && !error && eventCategories.length > 0 && filteredItems.length === 0 ? (
        <p className="mt-6 rounded-xl border border-[#2D2D2D] bg-[#151515] px-4 py-3 text-sm text-[#9C9C9C]">
