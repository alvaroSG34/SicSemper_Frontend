...
export const useParticipantNotifications = (): UseParticipantNotificationsResult => {
  const [isOpen, setIsOpen] = useState(false);
  const [items, setItems] = useState<ParticipantNotificationItem[]>([]);
  ...
  const fetchNotifications = useCallback(async (showLoading: boolean) => {
    ...
    const request = (async () => {
      ...
      try {
        const page = await participantService.listNotifications(
          DEFAULT_PAGE,
          DEFAULT_PAGE_SIZE,
        );
        setItems(page.items);
  ...
  const markAsRead = useCallback(async (notificationId: string) => {
    ...
    try {
      await participantService.markNotificationAsRead(notificationId);
      setItems((previous) =>
        previous.map((item) =>
  ...
  const markAllAsRead = useCallback(async () => {
    ...
    try {
      await participantService.markAllNotificationsAsRead();
  ...
  const deleteNotification = useCallback(async (notificationId: string) => {
    ...
    try {
      await participantService.deleteNotification(notificationId);
