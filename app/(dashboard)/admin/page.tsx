...
export default function AdminPage() {
  ...
  const isAdminPermissionManagerVisible = activeSection === "admins" && subtabParam === "permisos";
  const activePermissionAdminId = isAdminPermissionManagerVisible ? adminIdParam : null;
  const needsFullDashboardData =
    activeSection !== "inicio" && activeSection !== "ajustes" && adminAccess.section[activeSection];
  const permissionsResolved =
    isSuperadmin || effectivePermissions.length > 0 || (!!dashboard && !loading) || (!!summary && !loading);
  ...
  }, [activeSection, loadDashboard, needsFullDashboardData]);
  ...
  useEffect(() => {
    ...
    params.set("tab", firstAvailableSection);
    if (firstAvailableSection !== "admins") {
  ...
  const setActiveSection = (section: AdminSectionId) => {
    ...
    if (section === activeSection && !(section === "admins" && subtabParam)) {
      return;
    }
  ...
  const openAdminPermissionsManager = (targetUser: User) => {
    ...
    params.set("subtab", "permisos");
    params.set("adminId", targetUser.id);
    const query = params.toString();
    router.replace(query ? `${pathname}?${query}` : pathname, { scroll: false });
  };
  ...
  const setSelectedAdminPermissionTarget = (adminUserId: string) => {
    ...
    params.set("tab", "admins");
    params.set("subtab", "permisos");
    params.set("adminId", adminUserId);
    const query = params.toString();
    router.replace(query ? `${pathname}?${query}` : pathname, { scroll: false });
  };
  ...
  const {
    ...
    setPermissionSearch,
    filteredPermissions,
  } = useAdminPermissions(permissions);
